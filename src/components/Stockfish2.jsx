...
class Stockfish extends Component {
  ...
  engineGame = (options) => {
    ...
    function get_moves() {
      let moves = "";
      let history = this.props.game.history({ verbose: true });
    ...
    const prepareMove = () => {
      ...
      let turn = this.props.game.turn() === "w" ? "white" : "black";
      if (!this.props.game.game_over()) {
        // if (turn === playerColor) {
        if (turn !== playerColor) {
        ...
        if (
          this.props.game.history().length >= 2 &&
          !time.depth &&
          !time.nodes
        ) {
          startClock();
    ...
    engine.onmessage = (event) => {
      ...
      if (line === "uciok") {
        ...
        if ((match = line.match(/^info .*\bscore (\w+) (-?\d+)/))) {
          let score =
            parseInt(match[2], 10) * (this.props.game.turn() === "w" ? 1 : -1);
          ...
          if ((match = line.match(/\b(upper|lower)bound\b/))) {
            engineStatus.score =
              ((match[1] === "upper") === (this.props.game.turn() === "w")
                ? "<= "
                : ">= ") + engineStatus.score;
          }
        }
